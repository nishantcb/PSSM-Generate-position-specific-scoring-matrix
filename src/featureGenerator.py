...
def tpc(input_matrix):
    ...
    for i in range(20):
        matrix_tmp = [sum(pair) for pair in zip(matrix_final[i], matrix_tmp)]
...
def edp(input_matrix):
    ...
    for i in range(20):
        edp_matrix[0] = [sum(pair) for pair in zip(output_matrix[i], edp_matrix[0])]
...
def pssm_ac(input_matrix, GROUP):
    pssm_ac_matrix = correlation(input_matrix, 0, GROUP)
    return pssm_ac_matrix

def pssm_cc(input_matrix, GROUP):
    pssm_cc_matrix = correlation(input_matrix, 1, GROUP)
    return pssm_cc_matrix

'''
Feature encoding algorithms that simply combine the above single features
'''
